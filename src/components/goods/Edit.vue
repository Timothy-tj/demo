                ...
                <el-tabs v-model="activeIndex" tab-position="left" :before-leave="beforeTabLeave" @tab-click="tabClicked">
                    <el-tab-pane label="基本信息" name="0">
                        <el-form-item label="商品名称" prop="goods_name">
                            <el-input v-model.number="editForm.goods_name"></el-input>
                        </el-form-item>
...
export default {
    ...
    methods: {
        ...
        async getGoodsById(){
            ...
            //  转换成数组
            res.data.goods_cat = res.data.goods_cat.split(',').map(item => {  //字符串数组string的值转为number
                return item*1   
            })
            // 对图片数据回显
        ...
        editGoods(){
            this.$refs.editFormRef.validate(async valid => {
                if(!valid) return
                /**
                 * 发送请求前准备的数据editForm数据处理
                 */
                // 1.处理商品分类所属分类数组
                // this.editForm.goods_cat = this.editForm.goods_cat.join(',')    //修改成字符串了，导致级联选择器的goods_cat异常
                // 处理方法：定义一个新的editForm对象
                // const form = this.editForm        //浅拷贝,本质同一个对象，只是指针改变，实际未变
                // 使用lodash工具(Js库)的实现深拷贝，安装
                const form = _.cloneDeep(this.editForm)      //深拷贝（内容一样）
